      ...
      <el-row>
        <el-col :span="24" style=" margin-top: 20px;text-align: center">
          <el-table :data="allocationResults" style="width: 100%; height: 310px">
            <el-table-column prop="sku" label="名称" />
            <el-table-column prop="stock" label="可供量" />
...
export default {
  ...
  methods: {
    ...
    calculateDistribution() {
      ...
      this.allocationResults = this.skus.map((sku) => {
          ...
          for (let i = 1; i <= 999; i++) {
            for (let level = this.customerLevels - 1; level >= 0 && remainingStock > 0; level--) {
              ...
              if (level === this.customerLevels - 1 && sku.stock >= totalCustomers * 1.5) {
                ...
                const needAddFive =
                  this.customers[this.customerLevels - 1] * (lv1Number * this.ratio - lv30Number)
                if (lv30Number / lv1Number < this.ratio && remainingStock >= needAddFive) {
                ...
                if (sku.stock >= totalCustomers * 1.5) {
                  if (
                    level > 0 ||
                    remainingStock >=
                      this.customers[0] + this.customers[this.customerLevels - 1] * this.ratio
                  ) {
                    allocations[level] += levelCustomers
        ...
        return { sku: sku.sku, stock: sku.stock, allocations, remaining: remainingStock }
      })
      ...
      console.log(this.allocationResultsPer)
    },
    exportToCSV() {
      let csv =
        'SKU,库存量,' +
        Array.from({ length: this.customerLevels }, (_, i) => `${i + 1}档`).join(',') +
        ',剩余\n'
      this.allocationResultsPer.forEach((result) => {
        csv += `${result.sku},${result.stock},${result.allocations.join(',')},${result.remaining}\n`
      })

      const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
      const link = document.createElement('a')
      link.href = URL.createObjectURL(blob)
      link.setAttribute('download', 'distribution_result.csv')
