  ...
  <div class="container">
    <el-form :model="form" ref="formRef" :rules="formRules" label-width="150px">
      <el-row type="flex" justify="center" style="text-align: left">
        ...
        <el-col :span="9">
          <el-form-item label="业务科（市场部）：" prop="instCode">
            <el-select
              v-model="form.instCode"
              clearable
              placeholder="请选择业务科（市场部）"
              style="width: 85%; height: 30px"
            >
              <el-option
                v-for="item in instCodeOptions"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
      ...
      <el-row type="flex" justify="center">
        <el-col :span="24" style="text-align: center">
          ...
          <el-button type="warning" v-if="judgeType == '1'" @click="updateClick('0')"
            >关闭需求上报</el-button
          >
          <el-button type="warning" v-if="judgeType == '0'" @click="updateClick('1')"
            >开启需求上报</el-button
          >
        </el-col>
      </el-row>
...
export default {
  ...
  async created() {
    const response = await judgeDemandTime({ name: 'demand' }) // 调用 upload 函数并传入 payload
    this.judgeType = response.data.data
  },
  methods: {
    async updateClick(type) {
      const response = await updateDemandTime({ type: type, name: 'demand' })
      if (response.data.code == 200) {
        this.$message.success(response.data.message)
      } else {
        this.$message.error(response.data.message)
      }
      const response1 = await judgeDemandTime({ name: 'demand' }) // 调用 upload 函数并传入 payload
      this.judgeType = response1.data.data
    },
    async exportClick() {
      const payload = {
      ...
      const response = await exportDemandMxTotal(payload) // 调用 upload 函数并传入 payload
      if (response.data.code == 200) {
    ...
    fetchData() {
      ...
      setTimeout(async () => {
        if (validatestat) {
          ...
          const response = await searchDemandMxTotal(payload) // 调用 upload 函数并传入 payload
          if (response.data.code == 200) {
